...
def process_specific_dates(date_strings, start_date=None, end_date=None):
    ...
    for collection_date in date_strings:
        # 商品ごとの集計を実行
        process_product_summary(collection_date)
        
        # ジャンルごとの集計を実行
        process_genre_summary(collection_date)
...
def process_product_summary(collection_date):
    """指定された日付の商品日次集計を実行する"""
    # 商品ごとの集計クエリ
...
def process_genre_summary(collection_date):
    """指定された日付の動画ジャンル日次集計を実行する"""
    # ジャンルごとの集計クエリ
    genre_summary_query = """
