...
def update_affiliate_frontend_from_master() -> Dict[str, Any]:
    ...
    try:
        ...
        # バッチ番号が1（最初のバッチ）の場合、アフィリエイト動画の前処理
        if batch_number == 1:
            print("アフィリエイトバッチ1: 2週間以前のplayCountIncreaseリセット処理を開始")
            ...
            AND parent_account_type = 'アフィ'
            """
            
            affected_rows = execute_write_query(reset_query)
            
            reset_execution_time = (datetime.now() - reset_start_time).total_seconds()
            print(f"アフィリエイト動画playCountIncreaseリセット完了: {affected_rows}件更新、実行時間: {reset_execution_time}秒")
            
            # 2日前以前でplayCountIncreaseとplay_countが一致している動画の処理
            print("アフィリエイトバッチ1: 2日前以前で再生数と増加数が一致する動画の処理を開始")
            ...
            print(f"アフィリエイト動画playCountIncrease NULL設定完了: {null_affected_rows}件更新、実行時間: {null_reset_execution_time}秒")
            
            # 作成日が2日以内でplayCountIncreaseとplay_countが一致していない動画の処理
            print("アフィリエイトバッチ1: 作成日が2日以内で再生数と増加数が一致していない動画の処理を開始")
            ...
            print(f"アフィリエイト処理対象日付の条件: created_at >= {target_date} (現在の日付から2日前)")
            ...
            print(f"アフィリエイト動画playCountIncrease同期完了: {sync_affected_rows}件更新、実行時間: {sync_execution_time}秒")
