...
def sync_video_data(video_data: Dict) -> Dict[str, str]:
    ...
    try:
        ...
        # ハッシュタグ同期のためのPub/Subメッセージを送信
        if hashtags:  # ハッシュタグが存在する場合のみ送信
            try:
                publisher = pubsub_v1.PublisherClient()
                topic_path = publisher.topic_path(project_id, 'video-hashtags-sync')
                
                # メッセージデータの作成
                hashtag_message_data = {
                    'video_id': video_id,
                    'hashtags': hashtags,
                    'post_time': jst_time.strftime('%Y-%m-%d'),  # DATE型用にフォーマット
                    'type': 'hashtag_sync'
                }
                ...
                future = publisher.publish(
                    topic_path,
                    json.dumps(hashtag_message_data).encode('utf-8')
                )
                future.result()  # 送信完了を待機
                
                logger.info(f"Hashtag sync task published for video {video_id}")
            except Exception as e:
                logger.error(f"Failed to publish hashtag sync task: {str(e)}")
