...
class TikTokRepository:
    ...
    async def list_all_connections(self) -> List[TikTokUserConnection]:
        """Retrieve TikTok connections for all linked users."""
        print("[DEBUG] list_all_connections invoked")
        conn = None
        connections: List[TikTokUserConnection] = []
        try:
            conn = get_db_connection()
            ...
            print(f"[ERROR] list_all_connections exception: {str(e)}")
            raise
        finally:
            if conn:
                conn.close()
