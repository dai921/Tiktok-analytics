...
@router.get("/api/corporate-videos")
async def get_corporate_videos(
    request: Request,
    page: int = 1,
    limit: int = 50,
    ...
    print(f"Received corporate request with params: {request.query_params}")
    ...
    try:
        ...
        latest_date_result = fetch_one("SELECT MAX(created_at) as max_date FROM frontend_data WHERE (is_pr = 0 OR is_pr IS NULL) AND (account_type IS NOT NULL AND account_type != '')")
        ...
        print(f"Error in get_corporate_videos: {str(e)}")
