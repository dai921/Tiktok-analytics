...
async def get_product_trends(
    ...
    logger.info(f"product-trends API called with params: start_date={start_date}, end_date={end_date}, genres={genres}, metric={metric}, limit={limit}")
    print(f"product-trends API called with params: start_date={start_date}, end_date={end_date}, genres={genres}, metric={metric}, limit={limit}")

    genre_list = genres.split(',') if genres else []

    # Resolve default date range when none is provided
    try:
    ...
    try:
        ...
        logger.info(f"Returning {len(trend_data)} data points for {len(top_products)} products (limit={safe_limit})")
