...
async def get_hashtag_trends(
    ...
    """Hashtag trend data with per-day values."""

    logger.info(f"hashtag-trends API called with params: start_date={start_date}, end_date={end_date}, metric={metric}, parent_account_type={parent_account_type}, limit={limit}")

    print(f"hashtag-trends API called with params: start_date={start_date}, end_date={end_date}, metric={metric}, parent_account_type={parent_account_type}, limit={limit}")
    ...
    try:
        ...
        top_result = conn.execute(top_hashtags_query, params)

        top_rows = top_result.fetchall()

        top_hashtags = [row.hashtags for row in top_rows]
        ...
        if top_hashtags:
            ...
            hashtag_filter = f"AND hashtags IN ({', '.join(hashtag_placeholders)})"
        ...
        logger.info(f"Returning {len(trend_data)} hashtag trend rows (limit={safe_limit})")
        ...
        logger.error(f"Error in hashtag-trends API: {str(e)}", exc_info=True)

        print(f"Error in hashtag-trends API: {str(e)}")
        ...
        logger.info("hashtag-trends database connection closed")

        print("hashtag-trends database connection closed")
