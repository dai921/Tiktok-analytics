...
| コード名 | 関数名 | 行数 | 説明 |
|--------------|-------|------|------|
| genre_stats | get_genre_stats | 133~153 | 動画ジャンルの集計期間のTOP10動画を取得 |

### その他Cloud Function
| コード名 | 関数名 | 行数 | 説明 |
|--------------|-------|------|------|
| top100_videos_sync | update_genre_top100_by_date | 181~184 | 更新する場合既存のデータを削除 |
| top100_videos_sync | process_genre_summary | 188~215 | 動画ジャンルのTOP100動画をテーブルに共有する |
| manual_top100_sync | process_genre_top100 | 255~258 | 更新する場合既存のデータを削除 |
| manual_top100_sync | process_genre_summary | 262~291 | 動画ジャンルのTOP100動画をテーブルに共有する |
